import { Body, Controller, Delete, Get, HttpCode, HttpStatus, Param, ParseUUIDPipe, Post, Query, Res } from '@nestjs/common'
...
export class DocumentsController {
  ...
  generate(
    ...
    return this.documentsService.generateForEmployee(employeeId, dto, user)
  }
  ...
  async download(
    ...
    const file = await this.documentsService.getGeneratedDocumentFile(id, user, query.format)
    return reply
  ...
  async remove(
    ...
    await this.documentsService.deleteGeneratedDocument(id, user)
    return null
  }
}
